"""
PuBe source-folded energy-deposition analysis for the organic neutron detector.
...
Produces a single summary figure with four panels:
  ...
  (B) Energy deposited per event in the active organic layer    -> the deliverable
  (C) Recoil-proton initial-energy spectrum (n-p elastic)       -> the physics
  (D) The actual constructed detector geometry                  -> for the record

Reads the ROOT output and the geometry dump written by the simulation, so the
figure always documents the geometry that produced it.
...
ACTIVE_LAYER = "PEDOT_PSS"


def load_geometry(path):
    """Return the geometry-dump text (or a placeholder if it is missing)."""
...
def panel_source(ax, file):
    ...
    # Input table: column 2 is fluence-per-lethargy, so dN/dE is proportional to col2/E.
    E_eV, col2 = [], []
    ...
    dNdE = np.where(E_MeV > 0, np.array(col2) / E_MeV, 0.0)
    # Normalize the input curve to the sampled histogram over the fast region.
    ...
    ax.plot(centers, in_on_grid * scale, color="orange", lw=2,
            label="Input PuBe (col2/E)")
...
def panel_deposition(ax, file):
    """(B) Per-event energy deposited in the active organic layer."""
    edep_mev = file["EventSummary"]["GaNEnergyDeposit"].array(library="np")
    vals = edep_mev[edep_mev > 0]
    # Range covers the full PuBe reach: recoil protons stop in the 10 mm slab
    # and can deposit up to ~the maximum neutron energy (~11 MeV).
    ax.hist(vals, bins=240, range=(0, 12), histtype="stepfilled",
            alpha=0.6, color="seagreen")
    ax.set_yscale("log")
    ax.set_title(f"(B) Energy deposited per event in {ACTIVE_LAYER}")
...
def panel_recoils(ax, file):
    ...
    d = file["StepData"].arrays(keys, library="np")
    df = pd.DataFrame({
        "EventID": d["EventID"], "TrackID": d["TrackID"],
        "Particle": d["ParticleName"], "Process": d["CreatorProcess"],
        "Energy": d["KineticEnergy"],
    })
    recoils = df[(df["Particle"] == "proton") & (df["Process"] == "hadElastic")]
    if recoils.empty:
        ax.text(0.5, 0.5, "No recoil protons found", transform=ax.transAxes,
                ha="center", va="center")
    else:
        e_mev = recoils.groupby(["EventID", "TrackID"])["Energy"].max()
        ax.hist(e_mev, bins=240, range=(0, 12), histtype="step",
...
def main():
    ...
    panel_deposition(axes[0, 1], file)
    panel_recoils(axes[1, 0], file)
    panel_geometry(axes[1, 1], geom_text)

    fig.suptitle("PuBe source-folded response - organic neutron detector",
                 fontsize=14, fontweight="bold")
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    fig.savefig(OUT_PNG, dpi=130)
    print(f"Saved {OUT_PNG}")

    # Keep a copy of the geometry record next to the figure.
