...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    // Record the actual constructed geometry for the analysis/output.
    DumpGeometry();

    return physWorld;
}

void DetectorConstruction::DumpGeometry(const G4String& fileName) const
{
    std::ofstream out(fileName.c_str());
    if (!out.is_open()) {
        G4cout << "DetectorConstruction: WARNING could not write geometry file '"
               << fileName << "'" << G4endl;
        return;
    }

    out << "# Detector geometry (auto-generated by DetectorConstruction::DumpGeometry)\n";
    out << "# Neutrons travel +z and enter the stack at the smallest z.\n";
    out << "# World material : " << fWorldMaterial->GetName() << "\n";
    out << "# World size     : " << fWorldSizeXY/mm << " x " << fWorldSizeXY/mm
        << " x " << fWorldSizeZ/mm << " mm (x,y,z)\n";
    out << "# Stack thickness : " << fTotalDetectorThickness/mm << " mm in "
        << fDetectorMaterials.size() << " layer(s)\n";
    out << "#\n";
    out << "# " << std::left
    ...
    G4double currentZ = -fTotalDetectorThickness / 2.0;
    for (std::size_t i = 0; i < fDetectorMaterials.size(); ++i) {
        const MaterialInfo& m = fDetectorMaterials[i];
        G4double zMin = currentZ;
        G4double zMax = currentZ + m.thickness;
        currentZ = zMax;
    ...
    G4cout << "DetectorConstruction: wrote geometry to '" << fileName << "'" << G4endl;
}
